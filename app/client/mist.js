updateMistBadge = function(){
    ...
    if(typeof mist !== 'undefined' && (!conf || !conf.confirmedOwners.length)) {
        ...
        mist.menu.setBadge(EleTools.formatBalance(balance, '0.0 a','element') + ' ELE');
    }
};
...
updateMistMenu = function(){
    ...
    var accounts = _.union(Wallets.find({}, {sort: {name: 1}}).fetch(), EleAccounts.find({}, {sort: {name: 1}}).fetch());
    ...
    Meteor.setTimeout(function(){
        ...
        _.each(accounts, function(account, index){
            mist.menu.add(account._id,{
                position: 3 + index,
                name: account.name,
                badge: EleTools.formatBalance(account.balance, "0 a", 'element')+ ' ELE',
