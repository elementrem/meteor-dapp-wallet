...
@class [template] elemens_transactions_table
@constructor
*/
...
@type Number
*/
var blocksForConfirmation = elementremConfig.requiredConfirmations;
...
Template['elemens_transactions_table'].onCreated(function(){
    this._properties = {
        cursor: {}
    };
...
Template['elemens_transactions_table'].helpers({
    /**
    ...
    'items': function(){
        ...
        if(searchQuery) {
            ...
            items = _.filter(items, function(item){
                ...
                // search value
                if(pattern.test(EleTools.formatBalance(item.value, '0,0.00[000000] unit')))
                    return item;
...
Template['elemens_transactions_table'].events({
...
@class [template] elemens_transactions_row
@constructor
*/


Template['elemens_transactions_row'].helpers({
    /**
    ...
    */
    'incomingTx': function(account){
        var account = EleAccounts.findOne({_id: account}) || Wallets.findOne({_id: account});
        return !!((account && this.from !== account.address) ||
                  (!account && (EleAccounts.findOne({address: this.to}) || Wallets.findOne({address: this.to}))));
    },
    /**
    ...
    */
    'unConfirmed': function() {
        if(!this.blockNumber || !EleBlocks.latest.number)
            return {
                confirmations: 0,
                percent: 0
            };

        var currentBlockNumber = EleBlocks.latest.number + 1,
...
Template['elemens_transactions_row'].events({
    /**
    ...
    */
    'click tr:not(.pending)': function(e) {
        var $elemen = $(e.target);
        if(!$elemen.is('button') && !$elemen.is('a')) {
