...
*/
var checkOverDailyLimit = function(address, mey, template){
    // check if under or over dailyLimit
    account = Helpers.getAccountByAddress(address);

    if(account && account.requiredSignatures > 1 && !_.isUndefined(account.dailyLimit) && account.dailyLimit !== elementremConfig.dailyLimitDefault && Number(mey) !== 0) {
        // check whats left
        var restDailyLimit = new BigNumber(account.dailyLimit || '0', 10).minus(new BigNumber(account.dailyLimitSpent || '0', 10));

        if(restDailyLimit.lt(new BigNumber(mey, 10)))
            TemplateVar.set('dailyLimitText', new Spacebars.SafeString(TAPi18n.__('wallet.send.texts.overDailyLimit', {limit: EleTools.formatBalance(restDailyLimit.toString(10)), total: EleTools.formatBalance(account.dailyLimit), count: account.requiredSignatures - 1})));
        else
            TemplateVar.set('dailyLimitText', new Spacebars.SafeString(TAPi18n.__('wallet.send.texts.underDailyLimit', {limit: EleTools.formatBalance(restDailyLimit.toString(10)), total: EleTools.formatBalance(account.dailyLimit)})));
    } else
...
Template['views_send'].onCreated(function(){
    ...
    Helpers.checkChain(function(error) {
        if(error && (EleAccounts.find().count() > 0)) {
    ...
    template.autorun(function(c){
        var unit = EleTools.getUnit();

        if(!c.firstRun && TemplateVar.get('selectedToken') === 'element') {
            TemplateVar.set('amount', EleTools.toMey(template.find('input[name="amount"]').value.replace(',','.'), unit));
        }
    });
});
...
Template['views_send'].onRendered(function(){
    ...
    template.autorun(function(c){
        ...
        // Element tx estimation
        if(tokenAddress === 'element') {

            if(EleAccounts.findOne({address: address}, {reactive: false})) {
...
Template['views_send'].helpers({
    ...
    */
    'reactiveData': function(deployContract){
        ...
        // Send funds
            ...
            TemplateVar.set('selectedToken', FlowRouter.getParam('token') || 'element');
        }
    },
    /**
    ...
    */
    'total': function(element){
        ...
        // element
        var gasInMey = TemplateVar.getFrom('.dapp-select-gas-price', 'gasInMey') || '0';

        if (TemplateVar.get('selectedToken') === 'element') {
            amount = (selectedAccount && selectedAccount.owners)
                ? amount
                : new BigNumber(amount, 10).plus(new BigNumber(gasInMey, 10));
        } else {
            amount = new BigNumber(gasInMey, 10);
        }
        return amount;
    },
    /**
    ...
    */
    ...
    /**
    Returns the total amount - the fee paid to send all element/coins out of the account
    ...
    'sendAllAmount': function(){
        ...
        if (TemplateVar.get('selectedToken') === 'element') {
            ...
            amount = (selectedAccount.owners)
                ...
                : new BigNumber(selectedAccount.balance, 10).minus(new BigNumber(gasInMey, 10)).toString(10);
        } else {
    ...
    */
    'requiresTokenApproval': function(){
        ...
        return tokenAddress != 'element' && replayProtection;
    }
});
...
Template['views_send'].events({
    ...
    */
    ...
    @event click .token-element
    */
    'click .token-element': function(e, template){
        TemplateVar.set('selectedToken', 'element');
    ...
    */
    'keyup input[name="amount"], change input[name="amount"], input input[name="amount"]': function(e, template){
        // element
        if(TemplateVar.get('selectedToken') === 'element') {
            var mey = EleTools.toMey(e.currentTarget.value.replace(',','.'));

            TemplateVar.set('amount', mey || '0');

            checkOverDailyLimit(template.find('select[name="dapp-select-account"].send-from').value, mey, template);
    ...
    */
    'submit form': function(e, template){
        ...
        if(selectedAccount && !TemplateVar.get('sending')) {
            ...
            if(sendAll && (selectedAccount.owners || tokenAddress !== 'element'))
                sendAll = false;
            ...
            if(selectedAccount.balance === '0' && (!selectedAccount.owners || tokenAddress === 'element'))
                return GlobalNotification.warning({
