...
addTransactionAfterSend = function(txHash, amount, from, to, gasPrice, estimatedGas, data, tokenId) {
    ...
    // add from Account
    EleAccounts.update({address: from}, {$addToSet: {
        transactions: txId
    }});

    // add to Account
    EleAccounts.update({address: to}, {$addToSet: {
        transactions: txId
    }});
};
...
addTransaction = function(log, from, to, value){
    ...
    var block = web3.ele.getBlock(log.blockNumber, false, function(err, block){
        if(!err) {

            web3.ele.getTransaction(log.transactionHash, function(err, transaction) {
                if(!err && transaction) {
                    web3.ele.getTransactionReceipt(log.transactionHash, function(err, receipt){
...
var updateTransaction = function(newDocument, transaction, receipt){
    ...
    if(receipt && transaction) {
        ...
        if(!newDocument.contractAddress && receipt.contractAddress) {
            web3.ele.getCode(receipt.contractAddress, function(e, code) {
                if(!e && code.length > 2) {
    ...
    if(newDocument.outOfGas) {
        ...
        if(EleAccounts.findOne({address: newDocument.from})) {
            web3.ele.getBalance(newDocument.from, newDocument.blockNumber, function(e, now){
                if(!e) {
                    web3.ele.getBalance(newDocument.from, newDocument.blockNumber-1, function(e, then){
...
observeTransactions = function(){
    ...
    */
    var checkTransactionConfirmations = function(tx){
        ...
        if(!tx.confirmed) {
            var filter = web3.ele.filter('latest');
            filter.watch(function(e, blockHash){
                if(!e) {
                    var confirmations = (tx.blockNumber && EleBlocks.latest.number) ? (EleBlocks.latest.number + 1) - tx.blockNumber : 0;
                    confCount++;
                    ...
                    if(confirmations < elementremConfig.requiredConfirmations && confirmations >= 0) {
                        ...
                        web3.ele.getTransaction(tx.transactionHash, function(e, transaction){
                            web3.ele.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                    ...
                    if(confirmations > elementremConfig.requiredConfirmations || confCount > elementremConfig.requiredConfirmations*2) {
                            ...
                            web3.ele.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                                if(!e) {
                                    ...
                                    if(!transaction || !transaction.blockNumber) {
                                        ...
                                        web3.ele.getBlock(transaction.blockNumber, function(e, block) {
                                            if(!e) {
    ...
    collectionObservers[collectionObservers.length] = Transactions.find({}).observe({
        ...
        */
        added: function(newDocument) {
            var confirmations = EleBlocks.latest.number - newDocument.blockNumber;
            ...
            if(!newDocument.exchangeRates || 
               ...
               !newDocument.exchangeRates.eur) {
                HTTP.get('https://min-api.cryptocompare.com/data/pricehistorical?fsym=ELE&tsyms=BTC,USD,EUR&ts='+ newDocument.timestamp, function(e, res){
        ...
        */
        removed: function(document) {
            ...
            EleAccounts.update({address: document.from}, {$pull: {
                transactions: document._id
            }});
            EleAccounts.update({address: document.to}, {$pull: {
                transactions: document._id
            }});
        }
    });
