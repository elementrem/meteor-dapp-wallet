...
Helpers.getDefaultContractExample = function(withoutPragma) {
    ...
    if (withoutPragma) {
        ...
        // so that older versions that will query code from wallet.elementrem.org won't use broken example code.
...
Helpers.getOwnedAccountFrom = function(accountList){
    ...
    var accounts = EleAccounts.find({}, {sort: {balance: 1}}).fetch();
...
Helpers.formatNumberByDecimals = function(number, decimals){
    ...
    return EleTools.formatNumber(new BigNumber(number, 10).dividedBy(Math.pow(10, decimals)), numberFormat);
};
...
Helpers.checkChain = function(callback){
    ...
    web3.ele.getCode(originalContractAddress, function(e, code){
...
Helpers.isWatchOnly = function(id) {
    return !Wallets.findOne({_id: id, owners: {$in: _.pluck(EleAccounts.find({}).fetch(), 'address')}});
};
...
/**
Gets the docuement matching the given addess from the EleAccounts or Wallets collection.
...
Helpers.getAccountByAddress = function(address, reactive) {
    ...
    return EleAccounts.findOne({address: address}, options) || Wallets.findOne({address: address}, options) || CustomContracts.findOne({address: address}, options);
};

/**
Gets the docuement matching the given query from the EleAccounts or Wallets collection.
...
Helpers.getAccounts = function(query, reactive) {
    ...
    return EleAccounts.find(query, options).fetch().concat(Wallets.find(query, options).fetch());
};

/**
Gets the docuement matching the given addess from the EleAccounts or Wallets collection and returns its name or address.
...
Helpers.formatTransactionBalance = function(value, exchangeRates, unit) {
    ...
    var unit = unit || EleTools.getUnit(),
        format = '0,0.00';
    ...
    if((unit === 'usd' || unit === 'eur' || unit === 'btc') &&
       exchangeRates && exchangeRates[unit]) {
        ...
        var price = new BigNumber(String(web3.fromMey(value, 'element')), 10).times(exchangeRates[unit].price);
        return EleTools.formatNumber(price, format) + ' '+ unit.toUpperCase();
    } else {
        return EleTools.formatBalance(value, format + '[0000000000000000] UNIT');
    }
};
...
Helpers.createTemplateDataFromInput = function (input, key){
    ...
    if(input.type.indexOf('[') === -1 &&
       (input.typeShort === 'string' ||
        ...
        input.template =  'elemens_input_'+ input.typeShort;
    } else {
        input.template =  'elemens_input_json';
    }
...
Helpers.getENSName = function(address, callback) {
    ...
    var ensContract = web3.ele.contract(ensContractAbi);
    var resolverContract = web3.ele.contract(resolverContractAbi);
