...
updateBalances = function() {
    ...
    _.each(walletsAndContracts, function(account){
        ...
        if(account.address) {
            web3.ele.getBalance(account.address, function(err, res){
                if(!err) {
                    // is of type wallet
    ...
    _.each(creatingWallets, function(wallet){
      ...
      web3.ele.getTransactionReceipt(wallet.transactionHash, function(error, receipt) {
    ...
    // UPDATE ENS
    var allAccounts = EleAccounts.find().fetch().concat(walletsAndContracts);
        ...
        if (!account.ensCheck || (account.ensCheck && now - account.ensCheck > 10*60*1000)) {
            Helpers.getENSName(account.address, function(err, name, returnedAddr) {
                ...
                if (!err && account.address.toLowerCase() == returnedAddr){
                    EleAccounts.update({address: account.address}, {$set:{ name: name, ens: true, ensCheck: now}});
                    ...
                    EleAccounts.update({address: account.address}, {$set:{ens: false, ensCheck: now}});
    ...
    var walletsContractsAndAccounts = EleAccounts.find().fetch().concat(Wallets.find().fetch());
...
observeLatestBlocks = function(){
    ...
    web3.ele.filter('latest').watch(function(e, res){
        if(!e) {
            updateBalances();
        }
    });
